...
drugs_review$rating_type <- ifelse(
  drugs_review$rating %in% c(9, 10), "positive",
    ifelse(
      drugs_review$rating %in% c(5, 6, 7, 8), "neutral",
        ifelse(
          drugs_review$rating %in% c(1, 2, 3, 4), "negative", NA)))
```

```{r}
# Plot the number of reviews for each rating type
ggplot(drugs_review, aes(x = rating_type)) +
  geom_bar() +
  labs(
    title = "Reviews by Rating Type",
    x = "Rating Type",
    y = "Number of reviews"
  ) +
  theme(
...
ggsave(
  "images/rating_type_frequency.png",
  width = 5,
  height = 4,
  dpi = 600,
)
